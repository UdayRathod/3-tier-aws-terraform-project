...
resource "aws_db_instance" "terracloud-rds-db" {
  ...
  username                    = "terracloud"
  manage_master_user_password = true #Integrating RDS master user passwords with Secrets Manager,RDS automatically generates database credentials & stores in AWS Secrets Manager.
  multi_az                    = true
